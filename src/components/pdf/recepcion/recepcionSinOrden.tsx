import React from "react";
import {
  Document,
  Text,
  Page,
  StyleSheet,
  Image,
  View,
} from "@react-pdf/renderer";
import { recepcionSOC } from "@/interfaces/recepcion.interface";

interface Props {
  data: recepcionSOC;
  location: { centrocosto?: string; bodega?: string; almacen?: string };
  estados: { codigo: number; nombre: string }[];
}

const LogoNetCoreBase64 =
  "data:image/png;base64,";

const styles = StyleSheet.create({
  page: {
    padding: 40,
    fontFamily: "Helvetica",
    fontSize: 11,
    color: "#333",
  },
  header: {
    display: "flex",
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
    marginBottom: 30,
    borderBottom: "2px solid #1a1a1a",
    paddingBottom: 10,
  },
  logo: {
    width: 160,
    height: 30,
  },
  headerText: {
    textAlign: "right",
    fontSize: 11,
    color: "#555",
  },
  title: {
    textAlign: "center",
    fontSize: 18,
    fontWeight: "bold",
    marginBottom: 25,
    color: "#111",
  },
  section: {
    display: "flex",
    flexDirection: "row",
    justifyContent: "space-between",
    marginBottom: 15,
  },
  col: {
    width: "48%",
  },
  label: {
    fontSize: 11,
    fontWeight: "bold",
    color: "#222",
  },
  text: {
    fontSize: 11,
    color: "#333",
    marginBottom: 6,
  },
  table: {
    display: "none",
    width: "100%",
    border: "1px solid #000",
    borderRight: 0,
    borderBottom: 0,
  },
  tableRow: {
    flexDirection: "row",
  },
  tableHeader: {
    backgroundColor: "#f4f4f4",
  },
  tableCol: {
    width: "20%",
    borderRight: "1px solid #000",
    borderBottom: "1px solid #000",
    padding: 4,
  },
  tableColFirst: {
    borderLeft: "1px solid #000",
  },
  tableTextHeader: {
    fontSize: 10,
    fontWeight: "bold",
    textAlign: "center",
  },
  tableText: {
    fontSize: 10,
    textAlign: "center",
  },
  signatureSection: {
    flexDirection: "row",
    justifyContent: "space-between", // 👈 separa las firmas a los extremos
    alignItems: "center",
    width: "100%", // 👈 que ocupe todo el ancho de la página
    marginTop: 80, // separa del contenido anterior
    paddingHorizontal: 60, // margen interno desde los bordes
  },
  signatureBlock: {
    alignItems: "center",
  },
  signatureLine: {
    borderBottomWidth: 1,
    borderColor: "#000",
    width: 190, // ancho de la línea de firma
    marginBottom: 6,
  },
  signatureLabel: {
    fontSize: 10,
    textAlign: "center",
  },
});

export default function PDFSinOrden({ data, location, estados }: Props) {
  return (
    <Document>
      <Page style={styles.page}>
        {/* Header */}
        <View style={styles.header}>
          <Image style={styles.logo} src={LogoNetCoreBase64} />
          <Text style={styles.headerText}>
            Fecha:{" "}
            {data.fecha.toLocaleDateString("es-ES", {
              year: "numeric",
              month: "long",
              day: "numeric",
            })}
          </Text>
        </View>

        {/* Title */}
        <Text style={styles.title}>Recepción sin Orden de Compra</Text>

        {/* Information Section */}
        <View style={styles.section}>
          <View style={styles.col}>
            <Text style={styles.label}>Centro de Costo</Text>
            <Text style={styles.text}>{location.centrocosto}</Text>

            <Text style={styles.label}>Bodega</Text>
            <Text style={styles.text}>{location.bodega}</Text>

            <Text style={styles.label}>Fecha del Documento</Text>
            <Text style={styles.text}>
              {data.fechaDoc.toLocaleDateString("es-ES", {
                year: "numeric",
                month: "long",
                day: "numeric",
              })}
            </Text>
          </View>

          <View style={styles.col}>
            <Text style={styles.label}>Tipo de Documento</Text>
            <Text style={styles.text}>
              {data.tipo === 2 ? "Guía de despacho" : "Factura"}
            </Text>

            <Text style={styles.label}>Número de Documento</Text>
            <Text style={styles.text}>{data.numDoc}</Text>
          </View>
        </View>

        {/* Table */}
        <View style={[styles.table, styles.tableHeader]}>
          <View style={styles.tableRow}>
            <View style={styles.tableCol}>
              <Text style={styles.tableTextHeader}>Código</Text>
            </View>
            <View style={styles.tableCol}>
              <Text style={styles.tableTextHeader}>Año</Text>
            </View>
            <View style={styles.tableCol}>
              <Text style={styles.tableTextHeader}>Nombre</Text>
            </View>
            <View style={styles.tableCol}>
              <Text style={styles.tableTextHeader}>Descripción</Text>
            </View>
            <View style={styles.tableCol}>
              <Text style={styles.tableTextHeader}>Estado recepción</Text>
            </View>
            <View style={styles.tableCol}>
              <Text style={styles.tableTextHeader}>Cantidad</Text>
            </View>
            <View style={styles.tableCol}>
              <Text style={styles.tableTextHeader}>Precio Compra</Text>
            </View>
            <View style={styles.tableCol}>
              <Text style={styles.tableTextHeader}>Total</Text>
            </View>
          </View>
        </View>

        {data.articulos?.map((art, i) => (
          <View style={styles.tableRow} key={i}>
            <View style={[styles.tableCol, styles.tableColFirst]}>
              <Text style={styles.tableText}>
                {art.codigo ? art.codigo : "-"}
              </Text>
            </View>
            <View style={styles.tableCol}>
              <Text style={styles.tableText}>{art.anoNumero}</Text>
            </View>
            <View style={styles.tableCol}>
              <Text style={styles.tableText}>{art.nombre}</Text>
            </View>
            <View style={styles.tableCol}>
              <Text style={styles.tableText}>
                {art.descripcion ? art.descripcion : "-"}
              </Text>
            </View>
            <View style={styles.tableCol}>
              <Text style={styles.tableText}>
                {estados.find((e) => e.codigo == art.estadoArticulo)?.nombre}
              </Text>
            </View>
            <View style={styles.tableCol}>
              <Text style={styles.tableText}>{art.cantidad}</Text>
            </View>
            <View style={styles.tableCol}>
              <Text style={styles.tableText}>${art.valor.toFixed(0)}</Text>
            </View>
            <View style={styles.tableCol}>
              <Text style={styles.tableText}>
                ${(art.cantidad * art.valor).toFixed(0)}
              </Text>
            </View>
          </View>
        ))}

        {/* Signature Section */}
        <View style={styles.signatureSection}>
          <View style={styles.signatureBlock}>
            <View style={styles.signatureLine} />
            <Text style={styles.signatureLabel}>Firma Encargado de Bodega</Text>
          </View>

          <View style={styles.signatureBlock}>
            <View style={styles.signatureLine} />
            <Text style={styles.signatureLabel}>Firma Recepción</Text>
          </View>
        </View>
      </Page>
    </Document>
  );
}
