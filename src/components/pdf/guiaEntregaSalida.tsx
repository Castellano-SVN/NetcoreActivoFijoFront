import React from "react";
import {
  Document,
  Page,
  Text,
  View,
  Image,
  StyleSheet,
} from "@react-pdf/renderer";
import { FormValueGuiaSalidaDetalle } from "@/interfaces/creation";

const LogoNetCoreBase64 =
  "data:image/png;base64,";

const styles = StyleSheet.create({
  page: {
    padding: 30,
    fontFamily: "Helvetica",
    fontSize: 11,
    color: "#000",
  },
  headerContainer: {
    display: "flex",
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
    borderBottom: "2px solid #1C002F",
    paddingBottom: 10,
    marginBottom: 15,
  },
  logo: {
    width: 180,
    height: 40,
  },
  dateText: {
    fontSize: 11,
    textAlign: "right",
  },
  title: {
    fontSize: 18,
    textAlign: "center",
    fontWeight: "bold",
    color: "#1C002F",
    marginVertical: 10,
  },
  section: {
    marginBottom: 8,
  },
  label: {
    fontWeight: "bold",
    color: "#1C002F",
  },
  row: {
    flexDirection: "row",
    justifyContent: "space-between",
    marginBottom: 4,
  },
  text: {
    fontSize: 11,
    color: "#000",
  },
  divider: {
    marginVertical: 10,
    borderBottom: "1px solid #aaa",
  },
  table: {
    width: "100%",
    border: "1px solid #000",
    borderRadius: 3,
  },
  tableHeader: {
    flexDirection: "row",
    backgroundColor: "#E8E8E8",
    borderBottom: "1px solid #000",
  },
  tableRow: {
    flexDirection: "row",
    borderBottom: "1px solid #ddd",
  },
  col: {
    padding: 5,
    borderRight: "1px solid #ddd",
    textAlign: "center",
    justifyContent: "center",
  },
  colHeader: {
    padding: 5,
    fontSize: 10,
    fontWeight: "bold",
    textAlign: "center",
  },
  footerSection: {
    flexDirection: "row",
    justifyContent: "space-between",
    marginTop: 40,
  },
  signature: {
    width: "40%",
    textAlign: "center",
  },
  signatureLine: {
    borderBottom: "1px solid #000",
    marginVertical: 10,
  },
});

interface Props {
  data: FormValueGuiaSalidaDetalle | null;
  estados: { codigo: number; nombre: string }[];
}

export default function PDFGuiaEntregaSalida({ data, estados }: Props) {
  const hoy = new Date().toLocaleDateString("es-ES", {
    weekday: "long",
    year: "numeric",
    month: "long",
    day: "numeric",
  });

  return (
    <Document>
      <Page style={styles.page}>
        {/* Header */}
        <View style={styles.headerContainer}>
          <Image style={styles.logo} src={LogoNetCoreBase64} />
          <Text style={styles.dateText}>Fecha: {hoy}</Text>
        </View>

        <Text style={styles.title}>Guía de Entrega de Salida</Text>

        {/* Información */}
        <View style={styles.section}>
          <View style={styles.row}>
            <Text style={styles.label}>Bodega:</Text>
            <Text style={styles.text}>{data?.BodegaNombre}</Text>
          </View>
          <View style={styles.row}>
            <Text style={styles.label}>Encargado:</Text>
            <Text style={styles.text}>{data?.FuncionarioEntregaNombre}</Text>
          </View>
          <View style={styles.row}>
            <Text style={styles.label}>Almacén:</Text>
            <Text style={styles.text}>{data?.AlmacenNombre}</Text>
          </View>
          <View style={styles.row}>
            <Text style={styles.label}>Destinatario:</Text>
            <Text style={styles.text}>{data?.PersonaRecibeNombre}</Text>
          </View>
        </View>

        <View style={styles.divider} />

        {/* Motivo */}
        <View style={styles.section}>
          <Text style={styles.label}>Motivo de la salida:</Text>
          <Text style={styles.text}>{data?.MotivoSalida || "—"}</Text>
        </View>

        {/* Observaciones */}
        <View style={styles.section}>
          <Text style={styles.label}>Observaciones:</Text>
          <Text style={styles.text}>{data?.Observacion || "—"}</Text>
        </View>

        <View style={styles.divider} />

        {/* Tabla de artículos */}
        <Text style={[styles.label, { marginBottom: 6 }]}>
          Artículos salidos:
        </Text>

        <View style={styles.table}>
          <View style={styles.tableHeader}>
            {[
              "Código artículo",
              "Nombre artículo",
              "Descripción",
              "Cantidad",
              "Estado",
              "Observaciones",
            ].map((header, i) => (
              <Text
                style={[styles.colHeader, { width: `${100 / 6}%` }]}
                key={i}
              >
                {header}
              </Text>
            ))}
          </View>

          {data?.GuiaSalidaDetalle.map((item, i) => (
            <View
              style={[
                styles.tableRow,
                { backgroundColor: i % 2 === 0 ? "#FAFAFA" : "#FFFFFF" },
              ]}
              key={i}
            >
              <Text style={[styles.col, { width: "16%" }]}>
                {item.CodigoArticulo ?? "—"}
              </Text>
              <Text style={[styles.col, { width: "18%" }]}>{item.nombre}</Text>
              <Text style={[styles.col, { width: "22%" }]}>
                {item.DescripcionArticulo ?? "—"}
              </Text>
              <Text style={[styles.col, { width: "10%" }]}>
                {item.Cantidad}
              </Text>
              <Text style={[styles.col, { width: "14%" }]}>
                {estados.find((e) => e.codigo === item.EstadoArticuloCodigo)
                  ?.nombre ?? "—"}
              </Text>
              <Text style={[styles.col, { width: "20%" }]}>
                {item.Observacion ?? "—"}
              </Text>
            </View>
          ))}
        </View>

        {/* Firmas */}
        <View style={styles.footerSection}>
          <View style={styles.signature}>
            <View style={styles.signatureLine} />
            <Text>Entregado por</Text>
          </View>
          <View style={styles.signature}>
            <View style={styles.signatureLine} />
            <Text>Recibido por</Text>
          </View>
        </View>
      </Page>
    </Document>
  );
}
