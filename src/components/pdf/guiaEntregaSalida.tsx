import React from 'react';
import { Document, Text, Page, StyleSheet, Image, View } from '@react-pdf/renderer'
import { FormValueGuiaSalidaDetalle } from '@/interfaces/creation';


const LogoNetCoreBase64 = 'data:image/png;base64,';

const styles = StyleSheet.create({
    page: {
        border: '2px solid #000'
    },
    img: {
        width: 250,
        height: 40
    },
    br: {
        marginVertical: 10
    },
    brpequeno: {
        marginVertical: 5
    },
    title: {
        fontSize: 17,
        textAlign: "left",
        fontWeight: "bold",
        marginVertical: 4,
        marginRight: 10,
        color: "#1C002F",
        fontStyle: "Nunito Sans",
    },
    title2: {
        fontSize: 17,
        textAlign: "right",
        fontWeight: "bold",
        marginVertical: 4,
        marginLeft: 10,
        color: "#1C002F",
        fontStyle: "Nunito Sans",
    },
    subTitle: {
        fontSize: 14,
        fontWeight: "bold",
        color: "black",
        fontStyle: "Nunito Sans"
    },
    sectionTitle: {
        display: "flex",
        flexDirection: "row",
        justifyContent: "center",
        padding: 4
    },
    sectionSubTitle: {
        display: "flex",
        flexDirection: "row",
        justifyContent: "space-between",
        marginHorizontal: 10,
    },
    columna: {
        display: "flex",
        flexDirection: "column",
    },
    fila: {
        display: "flex",
        flexDirection: "row",
    },
    parrafoSubTitle: {
        fontSize: 12,
        fontStyle: "Nunito Sans",
        color: "black",
        marginTop: 10,
        marginRight: 13,

    },
    parrafoSubTitle2: {
        fontSize: 12,
        fontStyle: "Nunito Sans",
        color: "black",
        marginTop: 10,
        marginLeft: 70,

    },
    parrafo: {
        fontSize: 12,
        fontStyle: "Nunito Sans",
        color: "black",
        marginTop: 1.5
    },
    parrafoObservacion: {
        fontSize: 12,
        fontStyle: "Nunito Sans",
        color: "black",
        marginTop: 1.5,
        width: '80%'
    },
    table: {
        width: '97%',
        border: '1px solid #000',
        margin: 10,
    },
    tableRow: {
        flexDirection: 'row',
        borderBottom: '1px solid #000',
        width: 575
    },
    tableColHeader: {
        width: '12.5%',
        borderRight: '1px solid #000',
        backgroundColor: '#f0f0f0',
    },
    tableCol: {
        width: '12.5%',
        borderRight: '1px solid #000',
    },
    tableCellHeader: {
        margin: 5,
        fontSize: 10,
        fontWeight: 'bold',
    },
    tableCell: {
        margin: 5,
        fontSize: 10,
    },
    subTitle2: {
        display: "flex",
        flexDirection: "row",
        justifyContent: "space-between",
        marginHorizontal: 10,
    },

    header: {
        position: 'absolute',
        top: 1,
        right: 20,
        display: 'flex',
        flexDirection: 'column',
        alignItems: 'flex-end',
    },
    columnaLeft: {
        display: "flex",
        flexDirection: "column",
        textAlign: 'left',
        justifyContent: 'space-between',
        alignItems: 'flex-start',
        padding: 10,
        width: '50%',

    },
    lineaRight: {
        borderBottom: '2px solid #000',
        width: '70%',
        alignSelf: 'flex-end',
        marginVertical: 20
    },
    textoDebajoLineaRight: {
        textAlign: 'right',
        fontSize: 12,
        fontStyle: 'Nunito Sans',
        color: 'black',
        marginTop: 2
    },
    lineaLeft: {
        borderBottom: '2px solid #000',
        width: '70%',
        alignSelf: 'flex-start',
        marginVertical: 20
    },
    textoDebajoLineaLeft: {
        textAlign: 'left',
        fontSize: 12,
        fontStyle: 'Nunito Sans',
        color: 'black',
        marginTop: 2
    },
    columnaRight: {
        display: "flex",
        flexDirection: "column",
        textAlign: 'right',
        justifyContent: 'space-between',
        alignItems: 'flex-end',
        padding: 10,
        width: '50%',
    },
});

interface props {
    data: FormValueGuiaSalidaDetalle | null;
}

export default function PDFGuiaEntregaSalida(props: props) {

    const hoy = new Date();
    const fechaHoy = hoy.toLocaleDateString('es-ES', {
        weekday: 'long',
        year: 'numeric',
        month: 'long',
        day: 'numeric'
    });



    return (
        <Document>
            <Page>
                <Image style={styles.img} src={LogoNetCoreBase64} />
                <View style={styles.header} >

                    <Text style={styles.parrafoSubTitle}>Fecha: {fechaHoy}</Text>
                </View>
                <View style={styles.sectionTitle} >
                    <Text style={styles.title}>Guía entrega de salida</Text>
                </View>


                <View style={styles.br}></View>

                <View style={styles.sectionSubTitle}>
                    <View style={styles.columna}>
                        <Text style={styles.subTitle}>Información:</Text>
                        <View style={styles.fila}>
                            <Text style={styles.parrafoSubTitle}>Bodega: {props.data?.BodegaNombre}</Text>
                        </View>
                        <View style={styles.fila}>
                            <Text style={styles.parrafoSubTitle}>Encargardo: {props.data?.FuncionarioEntregaNombre}</Text>
                        </View>
                    </View>
                    <View style={styles.columna}>
                        <Text style={styles.subTitle}> </Text>
                        <View style={styles.fila}>
                            <Text style={styles.parrafoSubTitle}>Almacen: {props.data?.AlmacenNombre}</Text>
                        </View>
                        <View style={styles.fila}>
                            <Text style={styles.parrafoSubTitle}>Destinatario: {props.data?.PersonaRecibeNombre}</Text>
                        </View>
                    </View>
                </View>

                <View style={styles.br}></View>

                <View style={styles.sectionSubTitle}>
                    <View style={styles.columna}>
                        <Text style={styles.subTitle}>Motivo de la salida:</Text>
                        <View style={styles.fila}>
                            <Text style={styles.parrafoSubTitle}>{props.data?.MotivoSalida}</Text>
                        </View>
                    </View>
                </View>

                <View style={styles.br}></View>

                <View style={styles.sectionSubTitle}>
                    <View style={styles.columna}>
                        <Text style={styles.subTitle}>Observaciones:</Text>
                        <View style={styles.fila}>
                            <Text style={styles.parrafoSubTitle}>{props.data?.Observacion}</Text>
                        </View>
                    </View>
                </View>

                <View style={styles.br}></View>

                <View style={styles.sectionSubTitle}>
                    <View style={styles.columna}>
                        <Text style={styles.subTitle}>Articulos salidos:</Text>
                    </View>
                </View>

                <View style={styles.table}>
                    <View style={styles.tableRow}>
                        <View style={styles.tableColHeader}>
                            <Text style={styles.tableCellHeader}>Código sub-familia</Text>
                        </View>
                        <View style={styles.tableColHeader}>
                            <Text style={styles.tableCellHeader}>Sub-familia</Text>
                        </View>
                        <View style={styles.tableColHeader}>
                            <Text style={styles.tableCellHeader}>Código artículo</Text>
                        </View>
                        <View style={styles.tableColHeader}>
                            <Text style={styles.tableCellHeader}>Descripción artículo</Text>
                        </View>
                        <View style={styles.tableColHeader}>
                            <Text style={styles.tableCellHeader}>Cantidad sistema</Text>
                        </View>
                        <View style={styles.tableColHeader}>
                            <Text style={styles.tableCellHeader}>Cantidad salida</Text>
                        </View>
                        <View style={styles.tableColHeader}>
                            <Text style={styles.tableCellHeader}>Estado Articulo</Text>
                        </View>
                        <View style={styles.tableColHeader}>
                            <Text style={styles.tableCellHeader}>Observaciones</Text>
                        </View>
                    </View>
                    {props.data?.GuiaSalidaDetalle.map((salida, index) => (
                        <View style={styles.tableRow} key={index}>
                            <View style={styles.tableCol} >
                                <Text style={styles.tableCell}>{salida.CodigoSubFamilia}</Text>
                            </View>
                            <View style={styles.tableCol}>
                                <Text style={styles.tableCell}>{salida.NombreSubFamilia}</Text>
                            </View>
                            <View style={styles.tableCol}>
                                <Text style={styles.tableCell}>{salida.CodigoArticulo ?? 'No disponible'}</Text>
                            </View>
                            <View style={styles.tableCol}>
                                <Text style={styles.tableCell}>{salida.DescripcionArticulo ?? 'No disponible'}</Text>
                            </View>
                            <View style={styles.tableCol}>
                                <Text style={styles.tableCell}>{salida.CantidadSistema}</Text>
                            </View>
                            <View style={styles.tableCol}>
                                <Text style={styles.tableCell}>{salida.Cantidad}</Text>
                            </View>
                            <View style={styles.tableCol}>
                                <Text style={styles.tableCell}>{salida.EstadoArticuloNombre == "" ? 'No disponible' : salida.EstadoArticuloNombre}</Text>
                            </View>
                            <View style={styles.tableCol}>
                                <Text style={styles.tableCell}>{salida.Observacion}</Text>
                            </View>
                        </View>
                    ))}
                </View>

                <View style={styles.br}></View>
                <View style={styles.fila}>
                    <View style={styles.columnaLeft}>

                        <View style={styles.fila}>
                            <View style={styles.lineaLeft}></View>
                        </View>
                        <Text style={styles.textoDebajoLineaLeft}>Nombre y Firma Encargado de Recepción</Text>
                    </View>

                    <View style={styles.columnaRight}>
                        <View style={styles.fila}>
                            <View style={styles.lineaRight}></View>
                        </View>
                        <Text style={styles.textoDebajoLineaRight}>Nombre y Firma Encargado de Salida</Text>
                    </View>
                </View>

            </Page>
        </Document>
    );
}