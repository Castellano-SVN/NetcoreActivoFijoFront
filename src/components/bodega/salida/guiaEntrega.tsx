...
export default function GuiaEntrega() {
  ...
  const OutPutSchema = z.object({
    EmpresaIdOrigen: z.string({
      required_error: "Campo requerido",
      invalid_type_error: "Tipo invalido",
    }),
    EmpresaIdDestino: z.string({
      required_error: "Campo requerido",
      invalid_type_error: "Tipo invalido",
    }),
    CentroCostoIdOrigen: z.string({
      required_error: "Campo requerido",
      invalid_type_error: "Tipo invalido",
    }),
    CentroCostoIdDestino: z.string({
      required_error: "Campo requerido",
      invalid_type_error: "Tipo invalido",
    }),
    BodegaIdOrigen: z.string({
      required_error: "Campo requerido",
      invalid_type_error: "Tipo invalido",
    }),
    BodegaIdDestino: z.string({
      required_error: "Campo requerido",
      invalid_type_error: "Tipo invalido",
    }),
    AlmacenIdOrigen: z.string({
      required_error: "Campo requerido",
      invalid_type_error: "Tipo invalido",
    }),
    AlmacenIdDestino: z.string({
      required_error: "Campo requerido",
      invalid_type_error: "Tipo invalido",
    }),
    BodegaOrigen: z.string({
      required_error: "Campo requerido",
      invalid_type_error: "Tipo invalido",
    }),
    BodegaDestino: z.string({
      required_error: "Campo requerido",
      invalid_type_error: "Tipo invalido",
    }),
  ...
  const metodos = useForm<OutPutFormValues>({
    resolver: zodResolver(OutPutSchema),
    defaultValues: { ParteSalida: [] },
  });
  const {
    register,
    handleSubmit,
  ...
  const [dataCentroCostoModal, setDataCentroCostoModal] = useState<
    ICentroCosto[]
  >([]);
  ...
  const getAllBodegasByEmpresaYCentroCostoModal = async () => {
    try {
      const dataModal3 = await api_getAllBodegaByEmpresaYCentroCosto(
        jwt,
        idEmpresa,
        CCIdDestino,
      );
  ...
  const getAllAlmacenByEmpByCenByBodModal = async () => {
    try {
      const dataModal4 = await api_getAllAlmacenByEmpByCenByBod(
        jwt,
        idEmpresa,
        CCIdDestino,
        BodegaIdDestino,
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="flex flex-col md:grid md:grid-cols-6 md:gap-4 lg:grid lg:grid-cols-4 lg:gap-4 mb-4">
            <div className="col-span-3 lg:col-span-2">
              <label className="block text-left mb-2" htmlFor="numeroDocumento">
                Bodega origen:
              </label>
              ...
              {errors.BodegaOrigen && (
                <span className="text-red-600 block">
                  {errors.BodegaOrigen.message}
                </span>
              )}

              <button
                type="button"
                className="btn btn-outline btn-accent w-1/2 mt-2"
                onClick={handleShow3}
              >
            ...
            <Modal ref={ref3}>
              <Modal.Header className="font-bold">
                Seleccione la bodega y almacén de Origen
              </Modal.Header>
              <Modal.Body>
                <div className="flex flex-col md:grid md:grid-cols-4 md:gap-4 lg:grid lg:grid-cols-4 lg:gap-4 mb-4">
                  <div className="col-span-2">
                    ...
                    {errors.CentroCostoIdOrigen && (
                      <span className="text-red-600">
                        {errors.CentroCostoIdOrigen.message}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="col-span-2">
                    ...
                    <select
                      className="mt-1 block w-full py-2 px-3 border border-primary bg-white rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm"
                      {...register("BodegaIdOrigen", {
                        setValueAs: (value) =>
                          value === "" ? undefined : value,
                      })}
                      onChange={(e) => {
                        const selectedBodegaOrigen = dataBodega.find(
                          (bodegaOrigen) => bodegaOrigen.id === e.target.value,
                        );
                    ...
                    {errors.BodegaIdOrigen && (
                      <span className="text-red-600">
                        {errors.BodegaIdOrigen.message}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="col-span-2">
                    ...
                    {errors.AlmacenIdOrigen && (
                      <span className="text-red-600">
                        {errors.AlmacenIdOrigen.message}
                      </span>
                    )}
                  </div>
                </div>
            ...
            <Modal ref={ref}>
              <Modal.Header className="font-bold">
                Seleccione la bodega y almacén de destino
              </Modal.Header>
              <Modal.Body>
                <div className="flex flex-col md:grid md:grid-cols-4 md:gap-4 lg:grid lg:grid-cols-4 lg:gap-4 mb-4">
                  <div className="col-span-2">
                    ...
                    >
                      ...
                      {dataCentroCostoModal.map((centroCostoModal, index) => (
                        <option key={index} value={centroCostoModal.id}>
                          {centroCostoModal.nombre}
                        </option>
                      ))}
                    </select>
                    {errors.CentroCostoIdDestino && (
                      <span className="text-red-600">
                        {errors.CentroCostoIdDestino.message}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="col-span-2">
                    ...
                    <select
                      className="mt-1 block w-full py-2 px-3 border border-primary bg-white rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm"
                      {...register("BodegaIdDestino", {
                        setValueAs: (value) =>
                          value === "" ? undefined : value,
                      })}
                      onChange={(e) => {
                        const selectedBodega = dataBodegaModal.find(
                          (bodega) => bodega.id === e.target.value,
                        );
                        if (selectedBodega) {
                    ...
                    >
                      ...
                      {dataBodegaModal.map((bodegaModal, index) => (
                        <option key={index} value={bodegaModal.id}>
                          {bodegaModal.nombre}
                        </option>
                      ))}
                    </select>
                    {errors.BodegaIdDestino && (
                      <span className="text-red-600">
                        {errors.BodegaIdDestino.message}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="col-span-2">
                    ...
                    >
                      ...
                      {dataAlmacenModal.map((almacenModal, index) => (
                        <option key={index} value={almacenModal.id}>
                          {almacenModal.nombre}
                        </option>
                      ))}
                    </select>
                    {errors.AlmacenIdDestino && (
                      <span className="text-red-600">
                        {errors.AlmacenIdDestino.message}
                      </span>
                    )}
                  </div>
                </div>
                      ...
                      <Table.Row key={index} hover={true}>
                          ...
                          onChange={(e) => {
                            if (e.target.checked) {
                              append({
                                ...
                                CodigoFamilia:
                                  almacenArticulo.articulo.subFamilium.familium
                                    .codigo,
                                Familia:
                                  almacenArticulo.articulo.subFamilium.familium
                                    .nombre,
                        ...
                        {fields.find(
                          ...
                          <input
                            ...
                            {...register(
                              `ParteSalida.${fieldsIndex}.Cantidad`,
                              {
                                setValueAs: (value) =>
                                  value === "" ? undefined : Number(value),
                              },
                            )}
                            className={`bg-white mt-1 block w-full py-1 md:py-2 lg:py-2 px-3 border rounded-md shadow-sm focus:outline-none sm:text-sm
                                    ${
                                      errors.ParteSalida?.[fieldsIndex]
                                        ?.Cantidad
          ...
          {showPdf && dataPost ? (
            <Modal open={showPdf}>
              <Modal.Header>
                ¿Desea crear un reporte de la guía de despacho?
              </Modal.Header>
              <Modal.Body>
                <div className="flex flex-col md:grid md:grid-cols-4 md:gap-4 lg:grid lg:grid-cols-4 lg:gap-4 mb-4">
                  <div className="col-span-2">
                    <PDFDownloadLink
                      document={<PDFGuiaEntrega data={dataPost} />}
                      fileName={`Pdf_guia_de_entrega_bodega_${nameBodega}_a_bodega_${nameBodegaModal}`}
                    >
