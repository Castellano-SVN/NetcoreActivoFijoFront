...
import { api_getArticulos } from "@/services/bodega.service";
...
export default function Page(props: props) {
    ...
    return (
        <React.Fragment>
            ...
            {noItems ? (
                <>
                    <WarningAlert message="No existen articulos vinculos a esta empresa" />
                    <button
                    ...
                    {data?.pages?.map((page, pageIndex) => (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 w-full mt-2" key={pageIndex}>
                            {page.dataList.map((articulo: IArticulo, index: number) => (
                                <Element element={articulo} key={index} />
                            ))}
                        </div>
                    ))}
                </div>
...
function Element({ element }: { element: IArticulo }) {
    const router = useRouter();
